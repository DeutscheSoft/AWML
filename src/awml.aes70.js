...
var f = function (w, AWML) {
  ...
  function map_all_objects() {
    ...
    return this.device.GetDeviceTree().then(
      function (tree) {
        return get_all_roles(tree).then(
          function () {
            const rec = function (tree, path) {
              ...
              local_roles.forEach(
                function (o, role) {
                  ...
                  if (children.has(o)) rec(children.get(o), path);
                }.bind(this)
              );
            }.bind(this);
            rec(tree, '');
  ...
  aes70.prototype = Object.assign(Object.create(proto), {
      ...
      try {
        ...
        ws.addEventListener(
          ...
          function () {
            ...
            if (this.options.batch) options.batch = this.options.batch;

            this.device = new OCA.RemoteDevice(
              new OCA.WebSocketConnection(this.ws, options)
            );
            this.open();
          }.bind(this)
        );
    ...
    arguments_from_node: function (node) {
      ...
      if (batch !== null) options.batch = parseInt(batch);
    ...
    open: function () {
      ...
      map_all_objects.call(this).then(
        function () {
