...
(function (AWML) {
  ...
  function fetch_template(path, base) {
    ...
    return AWML.register_loading(
      AWML.fetch_text(url).then(function (text) {
  ...
  function get_template(O) {
    ...
    if (O.notemplate) {
      ...
      p = (O.cached ? fetch_template_cached : fetch_template)(
        O.handle,
        O.base_url
      );
    } else {
      ...
      p = el
        ? Promise.resolve(el)
        : Promise.reject(new Error('Could not find TEMPLATE element.'));
    }
  ...
  function deduplicate_to_head(node, selector, O) {
    ...
    for (var i = 0; i < elems.length; i++) {
      ...
      if (e.parentNode !== document.head) document.head.appendChild(e);
      if (e.tagName === 'link') e.addEventListener('load', onload);
    }
  ...
  function transform_template(node, O) {
    if (O.transform_template) node = O.transform_template(node);
  ...
  AWML.Tags.Clone = AWML.register_element(
    'awml-clone',
    Object.assign({}, AWML.PrefixLogic, AWML.RedrawLogic, {
      ...
      reload: function () {
        ...
        if (transform) transform = AWML.parse_format('js', transform);
      ...
      redraw: function () {
        ...
        while (node) {
          if (
            ...
            node.awml_data.handle === O.handle
          ) {
        ...
        pop_url();
        if (this.getAttribute('trigger-resize') !== null) onload();
      },
