...
mapping(string:object) backends = ([]);
...
class Backend {
    ...
    void simulate() {
        ...
        foreach (path2id; string path; int id) {
            if (search(path, "random") != -1) {
                changes += ({ id, random(1.0) });
            }
        }

        if (sizeof(changes)) receive(changes);
    }

    int get_id(string name) {
        int id;

        while (has_index(id2path, id = 1+random(sizeof(id2path)*2+1)));
    ...
    void receive(array data) {
        ...
        foreach (connections; object con; mapping subscriptions) {
